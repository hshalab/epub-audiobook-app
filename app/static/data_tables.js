(function () {
    var ENTRY_OPTIONS = [25, 50, 100, Infinity];
    var STORAGE_PREFIX = 'data-table-state:';
    var ACTION_TEXT = /^(actions?|thao\s*tac)$/i;
    ...
    function isActionColumn(table, index, header) {
        ...
        return rows.every(function (row) {
            ...
            return controlOnly(cell);
        });
    }

    function sortable(table, index, header) {
        return !isActionColumn(table, index, header) && !header.querySelector('input,select,button');
    }
    ...
    function isLinkColumn(rows, index) {
        ...
        return rows.slice(0, 20).every(function (row) {
            var cell = row.cells[index];
            return cell && cell.querySelectorAll('a').length > 0;
        });
    }
    ...
    function makeToolbar(table, state) {
        ...
        filters.className = 'data-table-filters';
        filterColumns(table).forEach(function (col) {
            var sel = document.createElement('select');
            sel.dataset.column = col.index;
        ...
        var entries = document.createElement('select');
        entries.setAttribute('aria-label', 'Rows per page');
        ...
        pagination.className = 'data-table-pagination pagination';
    ...
    function render(table) {
        ...
        var matches = table._dataRows.filter(function (row) {
            if (query && text(row).toLowerCase().indexOf(query) === -1) return false;
    ...
    function clearSort(table) {
        ...
        Array.from(headers).forEach(function (h) { delete h.dataset.sort; });
    }
